...
const AllTodoPage = async ({
  ...
  const totalOpenInProgressTodos = await db.todo.count({
    where: whereClause,
  });
  ...
  const totalPages = Math.ceil(totalOpenInProgressTodos / pageSize);
    ...
    <>
      ...
      <div className="flex justify-center items-center p-2 sm:p-4 md:p-6 w-full">
        <div className="w-full max-w-7xl">{todos && 
          <ToDoCard todos={todos} profile="all" users={users} 
